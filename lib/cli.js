  ...
  if (args.length > 0) {
    if (args.length > 3) {
      ...
      mdLinks(args[0], { validate: true })
        .then((links) => {
          links.forEach((link) => {
            console.log(chalk`{rgb(197, 153, 236).bold ${link.file}} {bold.underline ${link.href}} {white.bold.inverse ${link.ok}} {rgb(130, 226, 238).bold ${link.status}} {rgb(130, 226, 238).bold ${link.text}}`);
          });
        })
        .catch((err) => {
          console.log(error(err.message));
